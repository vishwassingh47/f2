...
public class UploadData
{
    ...
    public boolean getTinyDbData(TinyDB tinyDB)
    {
        ...
        if(isDataAvailable)
        {
            ...
            if(tinyDB.getString(code+"Url").length()>0)
            {
                downloadedSize+=tinyDB.getLong(code+"ChunkStart",0);
                ...
                {
                    progress=downloadedSize*100/totalSizeToDownload;
                }
            }
    ...
    public  void getPasterDataAndResetPasterAndTinyDB(TinyDB tinyDB)
    {
        ...
        tinyDB.remove(code+"Url");
        tinyDB.remove(code+"ChunkStart");
        tinyDB.remove(code+"IsRunning");
        tinyDB.remove(code+"CurrentSourcePath");
        tinyDB.remove(code+"LastSourcePath");
    }
}
